...
public class Input {
    public static void main(String[] args) {
        ...
        int[] pivots = new int[BigBlock];//装pivot的数组
        for (int i = 0; i < BigBlock; i++) {
            int bigBlock = sc.nextInt();
            pivots[i] = bigBlock;//往数组里面添加元素
            switch (sc.next()) {
                case "2*1":
                    c21.add(bigBlock);
                    break;
      ...
      //  传入GUI模块
        Game_Frame game = new Game_Frame(initial.length,initial[0].length);
        game.createChessboard(addMinus(initial,pivots));
        if(a!=null){
        game.showMove(a,pivots);
         }


    }

    public static int[][] addMinus(int[][] initial, int[] pivots){
        for (int i = 0; i < pivots.length; i++) {
            for (int j = 0; j < initial.length; j++) {
                for (int k = 0; k < initial[0].length; k++) {
                    if (pivots[i] == initial[j][k]){
                        initial[j][k] = -initial[j][k];
                    }
                }
            }
        }
